...
from core_clustering.dataset_dynamic_contrastive import dynamic_worker_init_fn, generate_entity_manifest
...
from core_clustering.trainer_contrastive_v3 import (
    DEFAULT_CONSISTENCY_PROB,
    DEFAULT_LAMBDA_GEOM,
    DEFAULT_LAMBDA_REF,
    ContrastiveTrainerV3,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="core-clustering-contrastive-pretrain-v3",
        description=(
            "V3 (see MTL_V3_REPORT.md): adds OPTIONAL local reference-set conditioning "
            "(K in {0,3,10,30,100} by default) and probabilistic (Laplace mean+scale) "
            "scalar outputs for Location/Extent/Intensity on top of V2.1's architecture. "
            "Same shared trunk/AttributeHeads/final L2 normalization; the reference "
            "context is fused before all four heads via a gated residual hard-forced to "
            "zero at K=0. Intensity's target is the raw, unbounded realized deviation D "
            "(no sigma-normalization, no bounded transform). Single AdamW over the whole "
            "model."
        ),
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    train_ds = EpisodicContrastiveDataset(entities, split="train", train=True, **dataset_kwargs)
    val_ds = EpisodicContrastiveDataset(entities, split="val", train=False, **dataset_kwargs)
    ...
    collate = functools.partial(episodic_pad_collate, max_len=args.max_len)

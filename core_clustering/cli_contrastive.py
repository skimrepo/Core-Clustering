...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="core-clustering-contrastive-pretrain",
        description=(
            "Multi-head contrastive pretraining: one dedicated embedding head "
            "per attribute (shape, location, extent, intensity), each read "
            "only by its own loss term -- shape uses a learnable-temperature "
            "contrastive loss; location regresses pairwise embedding "
            "distance toward a learnable scale times the true location gap; "
            "extent/intensity regress each anomaly's distance from the "
            "normal cluster toward a learnable scale times its own value "
            "(intensity in log-space). No fixed margins anywhere. Detection "
            "(masked-MSE + BCE, cli_tcn.py) is the downstream task that "
            "validates whether this pretraining produced useful "
            "representations."
        ),
    )
    ...
    parser.add_argument("--pooling", default="attention", choices=["attention", "mean"],
                         help="'attention': standard learned-query nn.MultiheadAttention pool + "
                              "sinusoidal positional encoding (default). 'mean': simple masked "
                              "mean-pool baseline, no attention/positional encoding.")
    ...
    parser.add_argument("--weight_shape", type=float, default=1.0,
                         help="how much this attribute's loss term matters in the combined gradient. "
                              "No margins: shape uses a learnable-temperature contrastive loss, "
                              "location/extent/intensity regress embedding distance toward a "
                              "learnable scale times the real attribute gap/value.")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    config = ConvBottleneckConfig(
        n_time_max=args.max_len, n_features=1, num_filters=num_filters,
    ...
    model = ContrastiveEncoder(config, embedding_dim=args.embedding_dim, pooling=args.pooling)
    weights = (args.weight_shape, args.weight_location, args.weight_extent, args.weight_intensity)
    trainer = ContrastiveTrainer(model, device=device, lr=args.lr, patience=args.patience,
